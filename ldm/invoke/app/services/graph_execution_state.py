...
from typing import Any, Callable, Dict, List, Literal, Optional, Union, get_args, get_origin, get_type_hints, Annotated
...
class Graph(BaseModel):
    ...
    def get_node(self, node_path: str) -> InvocationsUnion:
        ...
        if isinstance(node, GraphInvocation):
            graph = node.graph
            return graph.get_node(node_path[(len(node_id)+1):])
        else:
    ...
    def _get_input_edges(self, node_path: str, field: Optional[str] = None) -> list[tuple[EdgeConnection,EdgeConnection]]:
        ...
        edges.extend([e for e in self.edges if e[1].node_id == node_path and (field is None or e[1].field == field)])
